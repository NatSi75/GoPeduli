...
class GoPeduliPaginatedTableArticle extends StatelessWidget {
  const GoPeduliPaginatedTableArticle({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(
      () {
        ...
        Visibility(
          visible: false,
          child: Text(controller.filteredArticles.length.toString()),
        );
              ...
              columns: [
                ...
                const DataColumn2(
                    label: Text('Body',
                        style: TextStyle(color: GoPeduliColors.white))),
                DataColumn2(
                  label: const Text('Publish Date',
                      style: TextStyle(color: GoPeduliColors.white)),
                  onSort: (columnIndex, ascending) =>
                      controller.sortByPublishDate(columnIndex, ascending),
                ),
                DataColumn2(
                  label: const Text('Author',
                      style: TextStyle(color: GoPeduliColors.white)),
                  onSort: (columnIndex, ascending) =>
                      controller.sortByAuthor(columnIndex, ascending),
                ),
                DataColumn2(
                  label: const Text('Verified By',
                      style: TextStyle(color: GoPeduliColors.white)),
                  onSort: (columnIndex, ascending) =>
                      controller.sortByVerified(columnIndex, ascending),
                ),
                const DataColumn2(
                    label: Text('Action',
                        style: TextStyle(color: GoPeduliColors.white)),
